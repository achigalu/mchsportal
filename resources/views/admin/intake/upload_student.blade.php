...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">Upload Selection List</h4>
...
@endif
               <form action="{{route('uploaded.students')}}" method="POST" enctype="multipart/form-data">
               @csrf
                    <div class="row mb-3">
                ...
                <div class="row mb-3">
                    ...
                    @error('campus') <span class="text-danger">{{$message}}</span> @enderror
                        <select class="form-control select" name="campus" aria-label="Default select example">
                            
                        <option value="">-- select --</option>
                        @foreach($campus as $mycampus)
                            
                            <option value="{{$mycampus->id}}"> {{$mycampus->campus}}</option>
